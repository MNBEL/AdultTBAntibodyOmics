...
poc_data <- read_xlsx("/Users/sarahmali/Desktop/Luminex Ab Profiling Data_South Africa Cohort_FINAL.xlsx")
...
trainset <- data[,c(1:160)]
trainset
Y_train <- data$Y

which(colnames(trainset)=="LAM FcR1")
which(colnames(trainset)=="ESAT6 SNA")
which(colnames(trainset)=="ESAT6 FcR3B")
which(colnames(trainset)=="HSPX FcR3B")
which(colnames(trainset)=="Ag85A IgM")
#which(colnames(trainset)=="PSTS1 IgG")
#which(colnames(trainset)=="PPD IgG")
#which(colnames(trainset)=="LAM IgA")
#which(colnames(trainset)=="RV1363C IgG")



# Select only the top LASSO features and form the new training data set
new_trainset <- trainset[,c(1,113, 95, 155, 160, 84)]
new_trainset
...
#########################################################
library("pls")
POC_pls.fit = plsr(new_trainset$Y~., data = new_trainset[,c(2:6)], scale=TRUE)
summary(POC_pls.fit)
